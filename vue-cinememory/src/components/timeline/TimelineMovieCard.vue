<template>
  <div class="timeline-movie-card" :class="{ 'is-watched': isWatched }">
    <!-- 나이 표시 -->
    ...
    <div class="movie-poster">
      ...
      <div v-else class="poster-placeholder">
        <BaseIcon name="film" class="placeholder-icon" />
      </div>
      
      <!-- 시청 완료 오버레이 -->
      <div v-if="isWatched" class="watched-overlay">
        <BaseIcon name="check-circle" class="watched-icon" />
      </div>
    </div>
  ...
  methods: {
    ...
    async loadPosterImage() {
      try {
        ...
        if (this.movie.title) {
          const searchResults = await tmdbService.searchMovies(this.movie.title)
          if (searchResults.results && searchResults.results.length > 0) {
            const firstResult = searchResults.results[0]
            if (firstResult.poster_path) {
              this.posterUrl = tmdbService.getPosterUrl(firstResult.poster_path)
              return
            }
          }
        }
