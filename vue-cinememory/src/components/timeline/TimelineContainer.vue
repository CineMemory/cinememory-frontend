...
<template>
  <div class="timeline-container">
    ...
    <div v-else-if="error" class="error-wrapper">
      ...
      <BaseButton @click="loadTimeline" variant="primary">
        다시 시도
      </BaseButton>
    </div>
    ...
    <div v-else class="timeline-content">
      ...
      <div class="timeline-header">
        ...
        <p class="timeline-description">
          {{ timelineData.ageRange.min }}세부터 {{ timelineData.ageRange.max }}세까지의 추천 영화들
        </p>
      ...
      <!-- 타임라인 스크롤러 -->
      <div class="timeline-scroller" ref="timelineScroller">
        <!-- 나이 축 -->
        <div class="age-axis">
          <div
      ...
      <div class="progress-section">
        ...
        <p class="progress-text">
          {{ watchedMovies.size }} / {{ timelineData.totalItems }} 영화 시청 완료
  ...
  methods: {
    ...
    async loadTimeline() {
      ...
      try {
        ...
        this.error = error.message || '타임라인을 불러오는 중 오류가 발생했습니다.'
      } finally {
        this.loading = false
      }
    },
    ...
    scrollToAge(targetAge) {
      ...
      const targetItem = this.timelineData.items.find(item => item.age === targetAge)
...
.progress-fill {
  height: 100%;
  background: linear-gradient(90deg, var(--color-primary), var(--color-rating-good-text));
